...
async def get_login_args(session: SessionDep) -> list[SysArgModel]:
    """
    Lấy các tham số hệ thống thuộc nhóm "login" (toàn router ẩn khỏi Swagger).

    Không yêu cầu quyền admin vì cần dùng ở màn hình đăng nhập (ví dụ cấu hình SSO, hiển thị...).
    """
...
async def get_args(session: SessionDep) -> list[SysArgModel]:
    """
    Lấy toàn bộ tham số cấu hình hệ thống (chỉ admin).

    Dùng cho trang thiết lập tham số. Trả về danh sách ``SysArgModel``.
    """
...
async def save_args(session: SessionDep, request: Request):
    """
    Lưu (cập nhật) các tham số cấu hình hệ thống (chỉ admin).

    Nhận dữ liệu tham số từ request body và ghi xuống DB. Có ghi log thao tác.
    """
...
async def get_chat_args(session: SessionDep) -> list[SysArgModel]:
    """
    Lấy các tham số hệ thống thuộc nhóm "chat" (cấu hình liên quan tới hỏi đáp).

    Không yêu cầu quyền admin để phía giao diện chat có thể đọc cấu hình cần thiết.
    """
